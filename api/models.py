...
class TblAssetsManagement(models.Model):
    ...
    name_brand_category = models.ForeignKey(TblBrandCategories, on_delete=models.CASCADE,
                                            verbose_name=_("Name Brand Category"), null=True, blank=True)
    ...
    image_property = models.ImageField(upload_to="images/assets_awn", verbose_name=_('Image Property'), null=False,
                                       blank=False)
    name_category = models.ForeignKey(TblCategories, on_delete=models.CASCADE, verbose_name=_("Name Category"),
                                      null=False, blank=False)
    ram_size = models.CharField(max_length=10, choices=RAM_SIZE_CHOICES, verbose_name=_("RAM Size"), null=True,
                                blank=True)
    hard_disk_type = models.CharField(max_length=10, choices=Hard_Disk_Type_CHOICES, verbose_name=_("Hard Disk Type"),
                                      null=True, blank=True)
    capacity_hard_disk = models.CharField(max_length=10, choices=Capacity_Hard_Disk_CHOICES,
                                          verbose_name=_("Capacity Hard Disk Type"), null=True, blank=True)
...
class TblCustomUser(AbstractUser):
    ...
    company = models.ForeignKey(TblCompany, related_name="user_company", on_delete=models.CASCADE, null=True,
                                blank=True)
    department = models.ForeignKey(TblDepartment, related_name="user_department", on_delete=models.CASCADE, null=True,
                                   blank=False)
    signature_file = models.FileField(upload_to='signatures/', validators=[validate_pdf], null=True, blank=False)
    assets = models.ForeignKey(TblAssetsManagement, related_name="user_assets_management", on_delete=models.CASCADE,
                               null=True, blank=True)
